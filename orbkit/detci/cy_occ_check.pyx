...
def cis_aa(int i,int j, 
  ...
  cdef double f=0.0
  cdef int ia,im
  cdef int nacoeffs = acoeffs.shape[0]
  ...
  for ia in range(i,j):
    if acoeffs[ia] != 0.0:
      ...
      zc.append(zo)
      zm.append(si)
      for ib in range((ia+1),nacoeffs):
        if acoeffs[ib] != 0.0:
          if (aocc[ia,0] == aocc[ib,0]):
            si = []
            sc.append(2.0*acoeffs[ia]*acoeffs[ib])
            si.append(aocc[ia,1])
            si.append(aocc[ib,1])
            sm.append(si)
          elif (aocc[ia,1] == aocc[ib,1]):
            si = []
            sc.append(-2.0*acoeffs[ia]*acoeffs[ib])
            si.append(aocc[ib,0])
            si.append(aocc[ia,0])
...
def mcscf_ab(int i,int j, 
  ...
  cdef int alpha,beta
  cdef double f = 2.0 if identical_states else 1.0 #: Identical states
  cdef int warning = 0
  ...
  for ia in range(i,j):
    if acoeffs[ia] != 0.0:
      ib0 = ia if identical_states else 0
      for ib in range(ib0,nbcoeffs):
        if bcoeffs[ib] != 0.0:
          alpha = 0
          beta = 0
